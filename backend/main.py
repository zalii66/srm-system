...
# CORS中间件配置
# 开发环境：允许所有 localhost 端口（方便开发调试）
# 生产环境：使用配置的 CORS_ORIGINS
def is_localhost_origin(origin: str) -> bool:
    """检查origin是否是localhost或127.0.0.1（任意端口）"""
    if not origin:
        return False
    return origin.startswith("http://localhost:") or origin.startswith("http://127.0.0.1:")

def add_cors_headers(response: Response, origin: str, allowed_origins: list) -> None:
    """添加CORS响应头"""
    if origin and (is_localhost_origin(origin) or origin in allowed_origins):
...
if settings.DEBUG:
    # 开发环境：使用自定义CORS处理，允许所有localhost端口
    class FlexibleCORSMiddleware(BaseHTTPMiddleware):
        """灵活的CORS中间件，开发环境允许所有localhost端口"""
        async def dispatch(self, request: Request, call_next):
            origin = request.headers.get("origin")
            allowed_origins = list(settings.CORS_ORIGINS) if settings.CORS_ORIGINS else []
            
            # 开发环境：自动允许所有localhost端口
            if origin and is_localhost_origin(origin) and origin not in allowed_origins:
                allowed_origins.append(origin)
            
            # 处理OPTIONS预检请求
            if request.method == "OPTIONS":
                response = Response()
                add_cors_headers(response, origin, allowed_origins)
                return response
